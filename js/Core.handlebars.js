...
(function ($) {
    var compiled = {};
    $.fn.handlebars = function (template, data, Deferred) {
...
Handlebars.registerHelper('horarioFormat', function (date) {
    return `${date}:00`;
});
//
var _options;
$.get('views/helpers/options.hbs', function (data) {
    Handlebars.registerPartial("options", data);
    _options = data;
});

function View(view, data) {
    var Deferred = $.Deferred();
    $.get(`views/${view}`, data).then(function (tmp) {
        if (tmp) {
            $("#app").handlebars(tmp, data, Deferred);
            Deferred.resolve();
        } else {
            partialView("views/home/home.hbs");
            Deferred.fail(tmp);
        }
    });
    return Deferred.promise();
